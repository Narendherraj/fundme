  ...
  const approveCampaignHandler = () => {
    swal({
      ...
      if (willApprove) {
        closeModal();
        axios
          .put(`http://127.0.0.1:5000/updateStatus/${campaign.campaignId}`, {
            status: "active",
          })
          .then((response) => {
              ...
              {!admin ? null : (
                <div className={statsContainerClass}>
                  ...
                  <div className="flex flex-col w-full ml-2 items-start justify-evenly">
                    ...
                    {inReview || goalReached || !showApprove ? null : (
                      <p className="text-blue-200 text-sm">
                        <SubmitButton onClick={approveCampaignHandler}>
                          Approve
                        </SubmitButton>
                      </p>
                    )}
...
export default DonateModalInfoAdmin;
